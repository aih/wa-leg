...
| `deploy/deploy-remote.sh` | laptop or Actions | Fetch the deploy files for a git ref onto the box and run `bootstrap-box.sh`, through SSM (default) or SSH (`DEPLOY_METHOD=ssh`) |
...
| `deploy/iam-deploy-policy.json` | console | The Route 53, ECR and SSM grant attached to the deploy user |
...
and, from `deploy/iam-deploy-policy.json`, the `waleg` record in the hosted zone, push and pull on ECR
`waleg/*`, and `ssm:SendCommand` on instances tagged `Name=waleg-site`. It cannot create key pairs (the
SSH key goes in through cloud-init), cannot revoke security-group rules, and has no IAM, RDS, ECS or
Lightsail access. The repository secrets for the workflow are `AWS_ACCESS_KEY_ID` and
`AWS_SECRET_ACCESS_KEY`, the same user's key.
...
the images on an arm64 runner, pushes them to ECR, and runs `deploy-remote.sh`, which sends one SSM
command to the box: fetch that commit's `deploy/` files and run `bootstrap-box.sh` as `ubuntu`. The Actions tab also
...
DEPLOY_METHOD=ssh bash deploy/deploy-remote.sh "$TAG" "$(git rev-parse HEAD)"   # over SSH instead
```
...
- Without SSH: `aws ssm send-command --instance-ids <id> --document-name AWS-RunShellScript
  --parameters 'commands=["..."]'` runs a script as root on the box; `deploy-remote.sh` shows the
  pattern. `ssm:StartSession` is not granted.
- Rollback: `bash deploy/deploy-remote.sh <previous-tag> <previous-sha>`; ECR keeps every tag.
- The SSH rule allows one address. Add another with `aws ec2 authorize-security-group-ingress
  --group-name waleg-site --protocol tcp --port 22 --cidr <ip>/32`; removing one needs the console, since
  the deploy user cannot revoke rules.
